...
sed -i "s/<Connector port=\"\d+'\"/<Connector port=\"$port\"/g" "$TOMCAT_HOME/conf/server.xml"
sed -i "s/<Server port=\"\d+\" shutdown=\"SHUTDOWN\">/<Server port=\"$shutdownPort\" shutdown=\"SHUTDOWN\">/g" "$TOMCAT_HOME/conf/server.xml"
